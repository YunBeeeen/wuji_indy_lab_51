...
from .autotune.config import AutoTuneRequest, JointTuningContext, resolve_autotune_config
...
from .gui.autotune_panel import AutoTunePanel
...
class PdTunerWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, args: Any, package_root: Path) -> None:
        ...
        self.latest_telemetry: TelemetryPacket | None = None
        self.version_info: dict[str, Any] = {}
        self.autotune_resolved: dict[str, Any] = {}
        self.autotune_outcome: dict[str, Any] = {}
        self.autotune_original_gains: dict[str, Any] = {}
        self.autotune_showing_response = False
        self.autotune_active = False
        self._updating_joint = False
        # The child acknowledges joint changes asynchronously.  Keep only the
        # newest requested name so an older acknowledgement cannot move either
        # combo box back to a stale joint.
        self.pending_joint_selection: str | None = None
    ...
    def _build_ui(self) -> None:
        ...
        self.control_tabs.addTab(self._build_control_scroll(), "Manual Tune")
        self.autotune_panel = AutoTunePanel()
        self.control_tabs.addTab(self.autotune_panel, "Auto Tune")
    ...
    def _wire_ui(self) -> None:
        ...
        self.autotune_panel.joint_combo.currentIndexChanged.connect(self._autotune_joint_changed)
        self.autotune_panel.configurationChanged.connect(self._preview_autotune_configuration)
        self.autotune_panel.start_button.clicked.connect(self._start_autotune)
        self.autotune_panel.pause_button.clicked.connect(
            lambda: self._send_command(CommandKind.PAUSE_AUTOTUNE)
        )
        self.autotune_panel.resume_button.clicked.connect(
            lambda: self._send_command(CommandKind.RESUME_AUTOTUNE)
        )
        self.autotune_panel.cancel_button.clicked.connect(
            lambda: self._send_command(CommandKind.CANCEL_AUTOTUNE)
        )
        self.autotune_panel.apply_best_button.clicked.connect(self._apply_best_autotune)
        self.autotune_panel.apply_selected_button.clicked.connect(self._apply_selected_autotune)
        self.autotune_panel.restore_button.clicked.connect(
            lambda: self._send_command(CommandKind.RESTORE_ORIGINAL_GAINS)
        )
        self.autotune_panel.save_button.clicked.connect(self._save_autotune_result)
        self.autotune_panel.export_button.clicked.connect(self._export_autotune_csv)
        self.autotune_panel.table.itemSelectionChanged.connect(self._show_selected_autotune_response)
    ...
    def _start_simulation(self) -> None:
        ...
        self.pending_joint_selection = None
        try:
    ...
    def _send_command(self, kind: CommandKind, payload: dict[str, Any] | None = None) -> bool:
    ...
    def _handle_event(self, packet: EventPacket) -> None:
        if packet.kind == EventKind.STATE:
            ...
            self.version_info = dict(packet.payload)
            self.environment_text.appendPlainText(
            ...
            selected_payload = packet.payload.get("joint")
            if selected_payload is not None:
                acknowledged_name = str(selected_payload["name"])
                if "q0" in packet.payload:
                    self.joint_q0[acknowledged_name] = float(packet.payload["q0"])
                # SELECT_JOINT commands and their acknowledgements are FIFO, but
                # the user can change the combo faster than Isaac Sim replies.
                # Ignore every stale acknowledgement until the newest request
                # arrives; otherwise the combo appears impossible to select.
                if (
                    self.pending_joint_selection is not None
                    and acknowledged_name != self.pending_joint_selection
                ):
                    return
                self.pending_joint_selection = None
            if packet.payload.get("clear_graph"):
                self._clear_graph()
            if selected_payload is not None:
                name = str(selected_payload["name"])
                with QtCore.QSignalBlocker(self.joint_combo):
                    self.joint_combo.setCurrentIndex(self.joint_combo.findData(name))
                with QtCore.QSignalBlocker(self.autotune_panel.joint_combo):
                    auto_index = self.autotune_panel.joint_combo.findData(name)
                    self.autotune_panel.joint_combo.setCurrentIndex(max(auto_index, 0))
                self._display_joint(name)
            if "gains" in packet.payload:
                self._set_gain_controls(packet.payload["gains"])
            self._preview_autotune_configuration()
        ...
        elif packet.kind == EventKind.AUTOTUNE_STARTED:
            self.autotune_active = True
            self.autotune_resolved = dict(packet.payload.get("resolved_configuration", {}))
            self.autotune_original_gains = dict(packet.payload.get("original_gains", {}))
            self.autotune_outcome = {}
            self.autotune_showing_response = False
            self.autotune_panel.install_resolved(self.autotune_resolved)
            self.autotune_panel.set_progress({"state": "starting", **packet.payload})
            self.statusBar().showMessage("Auto Tune started.")
        elif packet.kind == EventKind.AUTOTUNE_CANDIDATE_STARTED:
            self.autotune_panel.set_progress({"state": "candidate_started", **packet.payload})
        elif packet.kind == EventKind.AUTOTUNE_PROGRESS:
            self.autotune_panel.set_progress(packet.payload)
        elif packet.kind == EventKind.AUTOTUNE_CANDIDATE_RESULT:
            self.autotune_panel.set_progress({"state": "candidate_complete", **packet.payload})
        elif packet.kind == EventKind.AUTOTUNE_COMPLETE:
            self.autotune_active = False
            self.autotune_resolved = dict(packet.payload.get("resolved_configuration", {}))
            self.autotune_original_gains = dict(packet.payload.get("original_gains", {}))
            self.autotune_outcome = dict(packet.payload.get("outcome", {}))
            self.autotune_panel.install_outcome(self.autotune_outcome)
            self.autotune_panel.set_progress(
                {
                    "state": "cancelled" if self.autotune_outcome.get("cancelled") else "complete",
                    "candidate_number": len(self.autotune_outcome.get("candidates", [])),
                    "search_budget": self.autotune_resolved.get("search_budget"),
                    "best_feasible_candidate_id": self.autotune_outcome.get("best_feasible_candidate_id"),
                    "best_fallback_candidate_id": self.autotune_outcome.get("best_fallback_candidate_id"),
                }
            )
            self.statusBar().showMessage("Auto Tune complete; gains were restored until Apply Best/Selected.", 10000)
        elif packet.kind == EventKind.AUTOTUNE_ERROR:
            self.autotune_active = False
            message = str(packet.payload.get("message", packet.payload))
            self.environment_text.appendPlainText(f"[AUTOTUNE ERROR] {message}")
            self.autotune_panel.progress_text.setText(f"ERROR: {message}")
    ...
    def _install_metadata(self, metadata: dict[str, Any]) -> None:
        ...
        old_selected = self.joint_combo.currentData()
        tunable_names = [joint["name"] for joint in metadata["joints"] if joint["tunable"]]
        selected = next(
            (
                name
                for name in (self.args.joint, old_selected)
                if name is not None and name in tunable_names
            ),
            tunable_names[0] if tunable_names else None,
        )
        # Populating a QComboBox changes its current index.  Blocking the signal
        # here prevents an unintended SELECT_JOINT command for the first row.
        with QtCore.QSignalBlocker(self.joint_combo):
            ...
            selected_index = self.joint_combo.findData(selected)
            self.joint_combo.setCurrentIndex(selected_index if selected_index >= 0 else -1)
        self.autotune_panel.populate_joints(metadata["joints"], selected)
        if selected is not None:
            self._display_joint(selected)
    ...
    def _joint_changed(self, index: int) -> None:
        ...
        if name:
            self._request_joint_selection(str(name))

    def _request_joint_selection(self, name: str) -> None:
        """Synchronize both selectors and request exactly one child update."""

        if name not in self.joint_metadata or not self.joint_metadata[name].get("tunable"):
            return
        with QtCore.QSignalBlocker(self.joint_combo):
            manual_index = self.joint_combo.findData(name)
            if manual_index >= 0:
                self.joint_combo.setCurrentIndex(manual_index)
        with QtCore.QSignalBlocker(self.autotune_panel.joint_combo):
            auto_index = self.autotune_panel.joint_combo.findData(name)
            if auto_index >= 0:
                self.autotune_panel.joint_combo.setCurrentIndex(auto_index)
        self._display_joint(name)
        self._preview_autotune_configuration()
        self.pending_joint_selection = name
        if not self._send_command(CommandKind.SELECT_JOINT, {"joint_name": name}):
            self.pending_joint_selection = None
    ...
    def _autotune_joint_changed(self, index: int) -> None:
        """Keep manual and automatic selectors on the same spawned joint."""

        if index < 0:
            return
        name = self.autotune_panel.joint_combo.itemData(index)
        if name:
            self._request_joint_selection(str(name))

    def _autotune_context(self) -> JointTuningContext:
        name = self.autotune_panel.joint_combo.currentData()
        if not name or name not in self.joint_metadata:
            raise ValueError("Start simulation and select a tunable joint first.")
        joint = self.joint_metadata[name]
        gains = self.current_gains.get(name, joint["original_gain"])
        if self.latest_telemetry is not None and self.latest_telemetry.joint_name == name:
            q0 = float(self.latest_telemetry.actual_position)
        elif name in self.joint_q0:
            q0 = self.joint_q0[name]
        else:
            raise ValueError("Waiting for the selected joint's simulator state/readback.")
        return JointTuningContext(
            joint_name=name,
            joint_index=int(joint["index"]),
            actuator_group=str(joint.get("actuator_group") or "unmapped"),
            lower_limit=float(joint["lower_limit"]),
            upper_limit=float(joint["upper_limit"]),
            velocity_limit=float(joint["velocity_limit"]),
            current_position=q0,
            current_kp=float(gains["stiffness"]),
            current_kd=float(gains["damping"]),
            current_effort_limit=float(gains["effort_limit"]),
            physics_dt=float(self.metadata["physics_dt"]),
        )

    def _resolve_autotune_from_panel(self) -> tuple[dict[str, Any], dict[str, Any]]:
        request_mapping = self.autotune_panel.request_mapping()
        request = AutoTuneRequest.from_mapping(request_mapping)
        resolved = resolve_autotune_config(request, self._autotune_context())
        return request_mapping, resolved.to_dict()

    def _preview_autotune_configuration(self) -> None:
        if not self.metadata:
            return
        try:
            _request, resolved = self._resolve_autotune_from_panel()
        except Exception as exc:
            self.autotune_panel.resolved_text.setPlainText(f"Configuration not ready: {exc}")
            return
        self.autotune_panel.install_resolved(resolved)

    def _start_autotune(self) -> None:
        if self.process is None or not self.process.is_alive():
            QtWidgets.QMessageBox.warning(self, "Auto Tune", "Start simulation first.")
            return
        try:
            request, resolved = self._resolve_autotune_from_panel()
        except Exception as exc:
            QtWidgets.QMessageBox.critical(self, "Invalid Auto Tune configuration", str(exc))
            return
        self.autotune_panel.install_resolved(resolved)
        allow_clipping = resolved.get("torque_policy") == "allow_clipping"
        torque_notice = (
            "Computed-effort clipping is ALLOWED. Actual applied effort remains limited, "
            "and saturation count/ratio will be reported."
            if allow_clipping
            else "Any torque saturation sample is a hard failure and will not be selected."
        )
        message = (
            "The following resolved configuration will be tested in Isaac Sim.\n\n"
            + self.autotune_panel.resolved_text.toPlainText()
            + f"\n\n{torque_notice}\nContinue?"
        )
        answer = QtWidgets.QMessageBox.question(self, "Confirm Auto Tune", message)
        if answer != QtWidgets.QMessageBox.StandardButton.Yes:
            return
        self.autotune_showing_response = False
        ...
        self.plots.clear()
        self._send_command(
            CommandKind.START_AUTOTUNE,
            {"joint_name": self.autotune_panel.joint_combo.currentData(), "request": request},
        )

    def _candidate_by_id(self, candidate_id: int | None) -> dict[str, Any] | None:
        if candidate_id is None:
            return None
        return next(
            (
                item
                for item in self.autotune_outcome.get("candidates", [])
                if int(item["candidate"]["candidate_id"]) == int(candidate_id)
            ),
            None,
        )

    def _apply_autotune_candidate(self, result: dict[str, Any] | None) -> None:
        if result is None:
            QtWidgets.QMessageBox.warning(self, "Apply Auto Tune", "No candidate is selected.")
            return
        if not result.get("hard_constraint_passed"):
            QtWidgets.QMessageBox.critical(
                self, "Unsafe candidate", "A hard-constraint-failed candidate cannot be applied."
            )
            return
        ...
        self._send_command(
            CommandKind.APPLY_GAINS,
            {
                "stiffness": float(candidate["kp"]),
                "damping": float(candidate["kd"]),
                "effort_limit": float(self.autotune_resolved["effort_limit"]),
            },
        )

    def _apply_best_autotune(self) -> None:
        self._apply_autotune_candidate(
            self._candidate_by_id(self.autotune_outcome.get("selected_candidate_id"))
        )

    def _apply_selected_autotune(self) -> None:
        self._apply_autotune_candidate(self.autotune_panel.selected_candidate())

    def _autotune_document(self) -> dict[str, Any]:
        if not self.autotune_outcome:
            raise ValueError("No completed Auto Tune result is available.")
        return build_autotune_document_from_payload(
            metadata=self.metadata,
            version_info=self.version_info,
            resolved_configuration=self.autotune_resolved,
            outcome=self.autotune_outcome,
            original_gains=self.autotune_original_gains,
        )

    def _save_autotune_result(self) -> None:
        try:
            document = self._autotune_document()
        except Exception as exc:
            QtWidgets.QMessageBox.warning(self, "Save Auto Tune", str(exc))
            return
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        default = self.outputs_root / "autotune" / f"autotune_{stamp}.json"
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save Auto Tune result", str(default), "JSON (*.json)"
        )
        if filename:
            save_json(filename, document)
            self.statusBar().showMessage(f"Saved Auto Tune result: {filename}", 5000)

    def _export_autotune_csv(self) -> None:
        if not self.autotune_outcome:
            QtWidgets.QMessageBox.warning(self, "Export Auto Tune", "No completed result is available.")
            return
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        default = self.outputs_root / "autotune" / f"autotune_candidates_{stamp}.csv"
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Export Auto Tune candidate summary", str(default), "CSV (*.csv)"
        )
        if filename:
            save_candidate_summary_payload_csv(filename, self.autotune_outcome)
            self.statusBar().showMessage(f"Exported Auto Tune CSV: {filename}", 5000)

    def _show_selected_autotune_response(self) -> None:
        result = self.autotune_panel.selected_candidate()
        if result is None or not result.get("representative_response"):
            return
        self.autotune_showing_response = True
        self.plots.show_autotune_response(
            result["representative_response"], float(self.autotune_resolved["effort_limit"])
        )

    def _refresh_plots(self) -> None:
        if not self.autotune_showing_response:
            self.plots.update_from_buffer(self.graph_buffer)
    ...
    def _session_document(self) -> dict[str, Any]:
        return {
            ...
            "autotune": self.autotune_panel.request_mapping(),
        }
    ...
    def _apply_session(self, session: dict[str, Any]) -> None:
        ...
        if isinstance(session.get("autotune"), dict):
            self.autotune_panel.load_request(session["autotune"])

...
class TelemetryPlots(QtWidgets.QWidget):
    ...
    def show_autotune_response(
        self,
        samples: list[dict[str, Any]],
        effort_limit: float,
    ) -> None:
        """Display one stored candidate response without altering live buffers."""

        times = [float(item["elapsed_time"]) for item in samples]
        targets = [float(item["target_position"]) for item in samples]
        positions = [float(item["actual_position"]) for item in samples]
        velocities = [float(item["joint_velocity"]) for item in samples]
        computed = [
            float(item["computed_effort"]) if item.get("computed_effort") is not None else float("nan")
            for item in samples
        ]
        applied = [
            float(item["applied_effort"]) if item.get("applied_effort") is not None else float("nan")
            for item in samples
        ]
        self.target_curve.setData(times, targets)
        self.position_curve.setData(times, positions)
        self.velocity_curve.setData(times, velocities)
        self.velocity_zero.setData(times, [0.0] * len(times))
        self.computed_curve.setData(times, computed)
        self.applied_curve.setData(times, applied)
        self.positive_limit_curve.setData(times, [effort_limit] * len(times))
        self.negative_limit_curve.setData(times, [-effort_limit] * len(times))

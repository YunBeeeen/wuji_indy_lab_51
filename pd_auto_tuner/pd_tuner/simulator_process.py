...
from .step_signal import PeriodicStepSignal, StepSample
...
def run_simulator_process(
    ...
    try:
        ...
        from .autotune.runtime import AutoTuneRuntime
        ...
        next_telemetry = 0.0
        state = "running"
        autotune_runtime: AutoTuneRuntime | None = None
        _send_event(
            event_queue,
        ...
        while running and not shutdown_event.is_set() and simulation_app.is_running():
            while True:
                ...
                try:
                    ...
                    if command.kind == CommandKind.PAUSE_SIMULATION:
                        ...
                        next_wall_step = time.perf_counter()
                        _send_event(event_queue, EventKind.STATE, {"state": state})
                    elif command.kind == CommandKind.PAUSE_AUTOTUNE:
                        if autotune_runtime is None or not autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Auto Tune is not running.")
                        autotune_runtime.pause()
                    elif command.kind == CommandKind.RESUME_AUTOTUNE:
                        if autotune_runtime is None or not autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Auto Tune is not running.")
                        autotune_runtime.resume()
                        next_wall_step = time.perf_counter()
                    elif command.kind == CommandKind.CANCEL_AUTOTUNE:
                        if autotune_runtime is None or not autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Auto Tune is not running.")
                        autotune_runtime.cancel(simulation_time)
                    elif command.kind == CommandKind.START_AUTOTUNE:
                        if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Auto Tune is already running.")
                        requested_joint = str(
                            command.payload.get("joint_name", robot.joint_names[selected_index])
                        )
                        if requested_joint not in adapter.list_tunable_joints():
                            raise ValueError(f"Joint is not tunable: {requested_joint}")
                        if requested_joint != robot.joint_names[selected_index]:
                            selected_index = list(robot.joint_names).index(requested_joint)
                        step_signal.pause()
                        tracker.reset()
                        current_gain = adapter.read_gains(selected_index)
                        joint_info = adapter.joint_infos[selected_index]
                        q0 = float(robot.data.joint_pos[0, selected_index].item())
                        request = AutoTuneRequest.from_mapping(
                            dict(command.payload.get("request", {}))
                        )
                        resolved = resolve_autotune_config(
                            request,
                            JointTuningContext(
                                joint_name=joint_info.name,
                                joint_index=joint_info.index,
                                actuator_group=joint_info.actuator_group or "unmapped",
                                lower_limit=joint_info.lower_limit,
                                upper_limit=joint_info.upper_limit,
                                velocity_limit=joint_info.velocity_limit,
                                current_position=q0,
                                current_kp=current_gain.stiffness,
                                current_kd=current_gain.damping,
                                current_effort_limit=current_gain.effort_limit,
                                physics_dt=cfg.physics_dt,
                            ),
                        )
                        ...
                        autotune_runtime = AutoTuneRuntime(
                            robot=robot,
                            adapter=adapter,
                            config=resolved,
                            target_positions=target_positions,
                            target_velocities=target_velocities,
                            event_sender=lambda kind, payload: _send_event(
                                event_queue, kind, payload
                            ),
                        )
                        autotune_runtime.start(simulation_time)
                    elif command.kind == CommandKind.SELECT_JOINT:
                        if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Cannot change joint while Auto Tune is running.")
                        ...
                        if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Manual gain updates are disabled while Auto Tune is running.")
                        ...
                        if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Cannot restore gains while Auto Tune is running; cancel it first.")
                        ...
                        if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Manual step input is disabled while Auto Tune is running.")
                        values = command.payload
                        ...
                        if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Cannot reset a joint while Auto Tune is running; cancel it first.")
                        step_signal.pause()
                        ...
                        if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                            raise RuntimeError("Cannot reset joints while Auto Tune is running; cancel it first.")
                        step_signal.pause()
                        _reset_robot(robot, initial_position, initial_velocity)
                    ...
                    auto_command = command.kind in (
                        CommandKind.START_AUTOTUNE,
                        CommandKind.PAUSE_AUTOTUNE,
                        CommandKind.RESUME_AUTOTUNE,
                        CommandKind.CANCEL_AUTOTUNE,
                    )
                    _send_event(
                        event_queue,
                        EventKind.AUTOTUNE_ERROR if auto_command else EventKind.ERROR,
            ...
            if paused:
                ...
                continue
            if (
                autotune_runtime is not None
                and autotune_runtime.blocks_manual_waveform
                and autotune_runtime.paused
            ):
                if cfg.render and not cfg.headless:
                    sim.render()
                else:
            ...
            joint_info = adapter.joint_infos[selected_index]
            if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                autotune_runtime.before_physics_step(simulation_time)
                auto_target = float(target_positions[0, selected_index].item())
                direction_name = autotune_runtime.progress_payload().get("direction", "positive")
                requested_target = autotune_runtime.config.requested_targets.get(
                    str(direction_name), auto_target
                )
                sample = StepSample(
                    requested_target=float(requested_target),
                    applied_target=auto_target,
                    clamped=autotune_runtime.config.target_clamped.get(
                        str(direction_name), False
                    ),
                    phase=f"autotune_{autotune_runtime.phase}",
                )
            else:
            ...
            if autotune_runtime is not None and autotune_runtime.blocks_manual_waveform:
                autotune_runtime.after_physics_step(
                    simulation_time,
            ...
            elif actual < joint_info.lower_limit - 1.0e-4 or actual > joint_info.upper_limit + 1.0e-4:
                safety_message = "Joint position exceeded its hard position limits."
            if saturated and not (
                autotune_runtime is not None and autotune_runtime.blocks_manual_waveform
            ):
                saturation_started = saturation_started or simulation_time
                if simulation_time - saturation_started >= cfg.saturation_safety_duration:
                    safety_message = (
                        f"Effort remained saturated for {cfg.saturation_safety_duration:.2f} s."
                    )
            else:
                saturation_started = None
            if safety_message:

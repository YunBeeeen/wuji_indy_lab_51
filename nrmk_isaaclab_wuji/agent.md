...
## 보상 설계 원칙 (2026-07-14 논의 확정)

- **Episode_Reward 비중은 회계 착시가 절반**: 비중 = 가중치 × 달성률 × 지속시간. 절대형 항(hold)은
  매 스텝 적립되므로, lift를 못 하는 동안엔 가중치와 무관하게 hold가 다 먹는 걸로 보임.
  비중 그래프를 교정 대상으로 삼지 말 것 — 볼 건 cube_clearance
- **보상 사다리 서열: reach < hold < lift < r_T. 역전 금지.** hold를 reach 아래로 내리면
  "다가가기 > 감싸기"가 되어 감싸기를 대충 하게 됨
- **hold 중앙 가상점의 관용은 논문(arXiv:2307.16752)의 의도된 설계**: "불완전하게 사이에 있어도
  양의 신호" (원문 인용 확인). 연금 문제의 마개는 점 수술이 아니라 **r_T(5000, 일회성, 성공 시
  에피소드 즉시 종료)** — 앉아서 버는 상한 < 성공 한 방 구조가 farming을 원천 봉쇄함
- **가벼운 물체일수록 접촉에 잘 튕김** (가속도 ∝ 1/질량). 들기 어렵다고 질량을 내리면
  밀어버림이 악화됨. 힘은 병목이 아니었음 (0.6 N·m로 300g 유지 실측)
- ⚠ **가중치 실험 시 클래스 확인**: Indy-Wuji-Cube-Grasp는 `CubeGraspRewardsCfg`를 씀
  (cube_grasp_env_cfg.py:90). `ChopsticksGraspRewardsCfg`는 어느 태스크에도 연결 안 된
  미사용 클래스 — 여기 고치면 조용히 무시됨 (2026-07-14 실제 발생: hold 12/lift 100이 미적용)

## 관찰된 실패 모드 (2026-07-14 커플링 런)

- **손가락 음수 관절 과신전**: 액션 목표가 default(0) ± 1.0 rad라 음수 명령 가능 → 손가락이
  뒤로 확 벌어짐 → 벌어진 채 바닥에 박힘 → kp 2로는 바닥 접촉을 못 이기고 못 접힘 → 에피소드
...
## 2026-08-21 ~ 08-24 — 105D 실물 배포 개괄

- `Deploy/` 재편, tip_link 관측 버그 수정, 105D 실물 파지 20초 완주.
- 명령 여백 0.95 를 계약 테이블에 반영. 정규화 범위는 공장표 그대로 — 정규화는
  학습과 같아야 하고, 명령만 좁힌다.
- 체크포인트를 갈아끼울 때 shape 검사는 거의 아무것도 안 지킨다. pregrasp·액션
  스케일·관절 순서·정규화·모드 스케줄은 전부 배포 쪽 상수다.
  `verify_policy_contract` 로 학습 런의 env.yaml 과 대조할 것.
- 전류 상한은 클램프지 레귤레이터가 아니다. 낮춰도 포화 시간은 안 줄고 열만 준다.
  진짜 손잡이는 정책이 만드는 위치 오차다.
- 실물 로그에 정책 경로가 안 남는다. obs 를 ONNX 에 재생하면 역추적은 되지만
  후보 파일이 지워지면 끝이다 — 메타 사이드카가 필요하다.

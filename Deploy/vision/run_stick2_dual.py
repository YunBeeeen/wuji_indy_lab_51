...
WORKSPACE_REFERENCE_CSV = (
    BASE_DIR
    / "references"
    / "stick2_workspace_reference_final.csv"
)
...
#
# IMPORTANT:
#   T_M2_S[:3, 3] = Stick center position expressed in Marker2 LOCAL frame
#   T_M3_S[:3, 3] = Stick center position expressed in Marker3 LOCAL frame
#
# This is intentionally symmetric with run_stick1_dual.py:
#   T_M2_S and T_M3_S are the two direct physical definitions.
#   T_M2_M3 is derived from them, never used as the source of truth.
# ============================================================
...
# Marker2 LOCAL frame -> Stick2 geometric center
...
# Marker3 -> Stick rotation preserved from the current calibrated geometry.
# Unlike the translation below, this does not need to change if Marker3 is
# only slid along the stick without changing its physical orientation.
T_M3_S[:3, :3] = np.array(
    [
        [-0.009235088, +0.046163577, -0.998891204],
        [-0.002968490, +0.998928136, +0.046192728],
        [+0.999952950, +0.003391792, -0.009088153],
    ],
    dtype=np.float64,
)

# Marker3 LOCAL frame -> Stick2 geometric center.
# CURRENT value reconstructed from the previous calibrated geometry.
# Replace these 3 numbers with the new direct physical measurement when
...
def load_workspace_orientation_references(csv_path):
    ...
    Marker A / Marker B의 CAMERA-frame quaternion들을 읽는다.
    ...
    print("CSV:", csv_path)
    print(
        f"Marker{MARKER_A_ID} references: "
        f"{len(refs[MARKER_A_ID])}"
    )
    print(
        f"Marker{MARKER_B_ID} references: "
        f"{len(refs[MARKER_B_ID])}"
    )
    print("=" * 80)
    print()
...
def build_marker_to_stick_transforms():
    ...
    # Derived only.  Source of truth is T_M2_S and T_M3_S above.
    T_M2_M3 = (
        T_M2_S
        @ invert_transform(
            T_M3_S
...
def select_physical_branch(
    ...
    """
    Generic physical IPPE branch selector for both Stick2 markers.
...
class DebugLogger:
    ...
    def __init__(
        ...
        self.path = (
            output_dir
            / f"stick2_online_handoff_{stamp}.csv"
        )
        ...
        self.fields = [
            ...
            # ================================================
            # NEW:
            # direct Marker A vs DUAL-implied Marker A
            # CAMERA FRAME
            # ================================================
            ...
            # ================================================
            # NEW:
            # direct Marker B vs DUAL-implied Marker B
            # CAMERA FRAME
            # ================================================
...
def main():
    ...
    REFERENCE_LOG_DIR = "/home/lsc/FoundationPose/stick2_debug_logs"
    os.makedirs(REFERENCE_LOG_DIR, exist_ok=True)

    reference_csv_path = os.path.join(
        REFERENCE_LOG_DIR,
        f"stick2_workspace_reference_"
    ...
    print("=" * 80)
    print(
        "STICK2 DUAL + SINGLE PNP DIAGNOSTIC"
    )
    print("=" * 80)
    ...
    print(
        "Pair geometry T_M2_M3 translation [mm]:"
    )
    ...
    print(
        f"Marker{MARKER_A_ID} -> Stick center [mm]:"
    )

    print(
        MARKER_TO_STICK[MARKER_A_ID][
            :3,
            3
        ]
        * 1000.0
    ...
    print(
        f"Marker{MARKER_B_ID} -> Stick center [mm]:"
    )

    print(
        MARKER_TO_STICK[MARKER_B_ID][
            :3,
            3
        ]
        * 1000.0
    ...
    try:
        ...
        while True:
            ...
            if dual is not None and dual.get("accepted", False):
                ...
                T_BASE_M0_TRUST = (
                    T_BASE_STICK_TRUST
                    @ invert_transform(MARKER_TO_STICK[MARKER_A_ID])
                )

                T_BASE_M1_TRUST = (
                    T_BASE_STICK_TRUST
                    @ invert_transform(MARKER_TO_STICK[MARKER_B_ID])
                )
                ...
                trusted_single[MARKER_A_ID] = True
                trusted_single[MARKER_B_ID] = True

                trusted_prev_marker_T[MARKER_A_ID] = (
                    T_CAMERA_M0_TRUST.copy()
                )

                trusted_prev_marker_T[MARKER_B_ID] = (
                ...
                for marker_id in (MARKER_A_ID, MARKER_B_ID):
            ...
            if key == ord("q"):
                ...
                if not rows_to_save:
                    ...
                    print(
                        f"Trusted M{MARKER_A_ID}    : "
                        f"{trusted_single[MARKER_A_ID]}"
                    )

                    print(
                        f"Trusted M{MARKER_B_ID}    : "
                        f"{trusted_single[MARKER_B_ID]}"
                    )
